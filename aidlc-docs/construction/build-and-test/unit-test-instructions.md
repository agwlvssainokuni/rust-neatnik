...
- **Expected**: 73 tests passed, 0 failures(2026-08-01時点の実測値)
- **内訳**(モジュール別、代表例):
  - `config`: 11件(YAML パース、BR-1/BR-2/BR-4バリデーション、パストラバーサル対策)
  - `scan`: 14件(include/exclude、FilenameDateRule、書き込み中判定、往復性)
  - `archive`: 17件(ArchiveNamer/BundleKey決定性、単体/バンドル圧縮、往復性)
  - `relocate`: 11件(コピー、mtime/パーミッション保持、衝突解決)
  - `delete`: 7件(セーフティブレーキ閾値評価)
  - `lock`: 4件(ジョブロックの取得・競合・解放)
  - `notify` / `clock` / `error`: 各2〜4件
  - `pipeline`: 8件(カスケード処理、in_use/猶予未経過スキップ、dry-run、セーフティブレーキ、バンドル統合、複数ジョブ)
- **Test Coverage**: 数値目標は設定していない(NFR Requirementsで規定なし)。PBT-02/03/07/08/09の対象箇所(business-rules.md 8章のテスト可能プロパティ表)はすべてテスト実装済み(詳細は`aidlc-docs/audit.md`のStep 4/6/7/8/14の記録を参照)
...
2026-08-01時点でいずれも警告・差分0件であることを確認済み。
